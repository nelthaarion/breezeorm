// Package sqlgen renders a compiled, optimized planner.PhysicalPlan into
// parameterized SQL text. It is a leaf package — it imports only dialect,
// planner, pool, and query — so that pkg/compiler can import it without a
// cycle (pkg/execution imports pkg/compiler, so pkg/compiler cannot import
// pkg/execution). Storing the rendered SQL text inside CompiledQuery (via
// the lazy SQL() method) eliminates the old planTextCache layer entirely.
//
...
func ExtractArgsFromBuilder[T any](b query.Builder[T]) ([]any, error) {
        ...
        case query.KindInsert, query.KindUpsert:
                // For INSERT/UPSERT we know the exact count: len(Assignments).
                // Pre-size exactly — no grow-and-copy.
                as := b.Assignments()
                c.args = make([]any, 0, len(as))
                c.assignments(as)
        case query.KindUpdate:
                // len(Assignments) + a few for the WHERE clause (4 is enough for
                // the common case of a single Eq/In/Between predicate; append()
                // grows amortized O(1) for any overflow).
                as := b.Assignments()
                c.args = make([]any, 0, len(as)+4)
                c.assignments(as)
                c.expr(b.WhereExpr())
        case query.KindDelete:
                ...
                c.expr(b.WhereExpr())
        default: // SELECT
                // Single-pass extraction: skip the old exprArgCount pre-count walk
                // (Task 2.3). Start with a generous default — most queries have
                // <8 args. append() grows amortized O(1), so 1-2 reallocations
                // for a complex WHERE is cheaper than a full second tree walk to
                // count args ahead of time.
                c.args = make([]any, 0, 8)
...
//
// This function is defined here in pkg/sqlgen (rather than pkg/execution,
// where it lived before Task 2.1's refactor) so that pkg/compiler can use
// it without an import cycle. pkg/execution re-exports it under the old
...
func GenerateBulkInsert(d dialect.Dialect, table string, columns []string, rows [][]any) (*GeneratedSQL, error) {
        if !d.BulkInsertSupported() {
                return nil, fmt.Errorf("sqlgen: dialect %s does not support bulk insert", d.Name())
        }
        if len(rows) == 0 {
                return nil, fmt.Errorf("sqlgen: bulk insert requires at least one row")
        }
        for i, r := range rows {
                if len(r) != len(columns) {
                        return nil, fmt.Errorf("sqlgen: row %d has %d values, want %d (len(columns))", i, len(r), len(columns))
                }
        }
