...
// — same eviction policy, same GetOrCompute miss-coalescing — but
// partitioned into N independently-locked LRU shards instead of one.
//
// Why this exists: LRU (see lru.go) serializes every Get/Set/GetOrCompute
// behind a single mutex. That's fine for a cache that's write-once,
// read-often from a *diverse* key space, but a prepared-statement cache
// under a benchmark like this repo's (GOMAXPROCS=12, every goroutine
// hammering the same handful of query shapes) has the opposite access
// pattern: a small, hot key set, hit concurrently from every P. A single
// mutex there serializes all 12 goroutines against each other on every
// single call regardless of key, which shows up directly as contention
// overhead in ns/op that has nothing to do with the actual prepare/query
// cost. Sharding by a hash of the key means two goroutines only contend if
// they happen to land in the same shard — with 32 shards and a handful of
// hot keys, that's rare.
//
// IMPORTANT — the hash is used ONLY to pick a shard, never as the map key
// itself. Each shard is a full LRU[K, V] keyed on the real K with exact
// equality. A hash collision therefore only costs a shard mis-pick (two
// unrelated keys sharing a shard, i.e. mild lock contention and a slightly
// smaller effective per-key capacity) — it can NEVER cause one key's lookup
// to return another key's value, which is what would happen if the hash
// were used as the identity itself. This distinction matters most for the
// prepared-statement cache: returning the wrong *Stmt for a SQL string
// would silently execute the wrong query.
