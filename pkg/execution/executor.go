...
type Executor struct {
        ...
        // stmtCache maps a query's uint64 CacheKey (the same shape-hash used by
        // compiledCache/planTextCache/scanPlanCache) to a prepared statement.
        // Bounded and
        ...
        stmtCache *cache.ShardedLRU[uint64, ormdriver.Stmt]
...
func newStmtCache(size, numShards int) *cache.ShardedLRU[uint64, ormdriver.Stmt] {
        return cache.NewShardedLRU[uint64, ormdriver.Stmt](size, numShards, cache.Uint64Hash, func(_ uint64, stmt ormdriver.Stmt) {
...
// Resolve turns a CompiledQuery into ready-to-execute SQL text, using the
// SQL-text cache on the CacheKey, combined with args extracted by the
// caller from its own current query.Builder (see ExtractArgsFromBuilder —
// args are never derived from cq.Physical here, since that tree is shared
// across every call for this query shape and is not safe to read per-call
// literal values from).
func (ex *Executor) Resolve(cq *compiler.CompiledQuery, args []any) (*GeneratedSQL, error) {
        ...
        if !ok {
                ...
                return &GeneratedSQL{SQL: gen.SQL, Args: args, CacheKey: cq.CacheKey}, nil
        }
        return &GeneratedSQL{SQL: sqlText, Args: args, CacheKey: cq.CacheKey}, nil
}

// prepare returns a cached prepared statement for the given query shape,
// preparing it (via sqlText) on first use. Keyed by the query's uint64
// CacheKey rather than the SQL text itself — the same O(1) shape key
// compiledCache/planTextCache/scanPlanCache already use — so this no longer
// hashes the full generated SQL string on every call. Concurrent misses for
// the same key are still coalesced within that key's shard (see
// cache.ShardedLRU.GetOrCompute), so a burst of concurrent first-time
// callers for a new query shape triggers exactly one PrepareContext round
// trip.
func (ex *Executor) prepare(ctx context.Context, cacheKey uint64, sqlText string) (ormdriver.Stmt, error) {
        return ex.stmtCache.GetOrCompute(cacheKey, func() (ormdriver.Stmt, error) {
...
func (ex *Executor) Query(ctx context.Context, gen *GeneratedSQL) (*Rows, error) {
        ...
        if ex.retry.MaxAttempts <= 1 {
                stmt, err := ex.prepare(ctx, gen.CacheKey, gen.SQL)
                if err != nil {
                        cancel()
