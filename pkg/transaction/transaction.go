...
func defaultIsRetryable(err error) bool {
	...
	for _, needle := range []string{"deadlock", "serialization failure", "lock wait timeout", "could not serialize"} {
...
func runNested(ctx context.Context, parent *Tx, fn func(ctx context.Context) error) error {
	parent.depth++
	spName := fmt.Sprintf("sp_%d", parent.depth)
	...
	nested := &Tx{sqlTx: parent.sqlTx, depth: parent.depth}
